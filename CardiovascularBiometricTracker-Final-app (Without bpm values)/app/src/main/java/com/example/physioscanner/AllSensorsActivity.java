...
public class AllSensorsActivity extends AppCompatActivity {
    ...
    private String socketUrl; // We'll set this dynamically from Firestore
...
//    private Button signInButton, logoutButton, homeButton;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        userDetails     = findViewById(R.id.user_info);
        //userDetails = findViewById(R.id.toolbar_title);
        ...
        homeButton.setOnClickListener(v -> {
            Intent intent = new Intent(AllSensorsActivity.this, CloudDatabaseActivity.class);
...
//        });
        ...
        DocumentReference piDocRef = db.collection("pi_status").document("status");

        // Attempt to get IP once
        piDocRef.get().addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                DocumentSnapshot doc = task.getResult();
                if (doc != null && doc.exists()) {
                    String piIp = doc.getString("ip");
                    if (piIp != null) {
                        socketUrl = "http://" + piIp + ":5000";
                        initSocket();
                    } else {
                        Log.d(TAG, "No 'ip' field found in Firestore doc");
                    }
                } else {
                    Log.d(TAG, "No doc or doc doesn't exist at pi_status/status");
                }
            } else {
                Log.w(TAG, "Error getting Pi IP from Firestore", task.getException());
            }
        });

        // Also listen for real-time changes
        piDocRef.addSnapshotListener((snapshot, e) -> {
            if (e != null) {
                Log.w(TAG, "Listen failed for pi_status/status", e);
                return;
            }
            if (snapshot != null && snapshot.exists()) {
                String newIp = snapshot.getString("ip");
                if (newIp != null) {
                    socketUrl = "http://" + newIp + ":5000";
                    Log.d(TAG, "Updated Pi IP: " + newIp);
                    // If already connected, you can optionally reconnect
    ...
    private void initSocket() {
        if (socketUrl == null) {
            Log.w(TAG, "initSocket called but socketUrl is null.");
            return;
        }
        ...
        try {
            ...
            Log.e(TAG, "Error creating socket with URL=" + socketUrl, e);
        }
    }
    ...
    private void addSocketListeners() {
        mSocket.on(Socket.EVENT_CONNECT, args -> {
            ...
            // Once connected, we emit 'login' with the current user's UID
            if (currentUser != null) {
                String uid = currentUser.getUid();
                mSocket.emit("login", uid);
                Log.d(TAG, "Emitted login event with uid=" + uid);
            }
        });
    ...
    protected void onResume() {
        super.onResume();
        if (mSocket != null && !mSocket.connected()) {
            Log.d(TAG, "Socket not connected in onResume, connecting...");
            mSocket.connect();
        }
    }
